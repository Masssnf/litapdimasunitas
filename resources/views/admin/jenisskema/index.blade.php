...
@section('header', 'Manajemen Jenis Skema')
        ...
        <div
            ...
            <div class="relative flex flex-wrap justify-between items-center gap-4">
                <div class="flex items-center space-x-4">
                    ...
                    <div>
                        <h1 class="text-2xl font-bold text-white tracking-tight">Manajemen Jenis Skema</h1>
                        <div class="flex items-center space-x-3 mt-0.5">
                            <span class="text-blue-100 text-sm">Kelola semua data jenis skema</span>
                            <span
                                class="px-2.5 py-0.5 bg-white/20 backdrop-blur-sm text-white text-xs font-semibold rounded-full">
                                {{ $jenisskema->total() }} Jenis Skema
                            </span>
                        </div>
                    </div>
                </div>

                <a href="{{ route('admin.jenisskema.create') }}"
                    ...
                    <span
                        class="absolute inset-0 bg-gradient-to-r from-blue-50 to-indigo-50 opacity-0 group-hover:opacity-100 transition-opacity duration-300"></span>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            <div
                ...
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-sm text-gray-500 font-medium">Total Jenis Skema</p>
                ...
                <div class="mt-3 pt-3 border-t border-gray-100">
                    <span class="text-xs text-gray-400">Total seluruh jenis skema</span>
                </div>
            </div>
            ...
            <div
                ...
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-sm text-gray-500 font-medium">Jenis Skema Aktif</p>
            ...
            <div
                ...
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-sm text-gray-500 font-medium">Jenis Skema Nonaktif</p>
        ...
        <div
            ...
            <div class="px-5 py-3.5 border-b border-gray-100 flex flex-wrap justify-between items-center gap-3">
                <div class="flex items-center space-x-4">
                    <div class="flex items-center space-x-2">
                        ...
                        <span class="text-sm font-bold text-gray-800">{{ $jenisskema->total() }}</span>
                    </div>
                ...
                <div class="flex items-center space-x-2">
                    <form method="GET" action="{{ route('admin.jenisskema.index') }}" class="flex items-center space-x-2">
                        <div class="relative">
                            <i class="fas fa-search absolute left-3.5 top-1/2 -translate-y-1/2 text-gray-400 text-xs"></i>
                            <input type="text" name="search" value="{{ request('search') }}"
                                placeholder="Cari jenis skema..."
                        ...
                        <button type="submit"
                            class="w-9 h-9 rounded-xl bg-blue-500 hover:bg-blue-600 text-white transition flex items-center justify-center">
                ...
                <table class="w-full">
                    <thead>
                        <tr class="bg-gray-50/50 text-gray-500 text-xs font-semibold uppercase tracking-wider">
                            ...
                            <th class="py-3.5 px-5 text-left">Nama Jenis Skema</th>
                    ...
                    <tbody class="divide-y divide-gray-50">
                        @forelse($jenisskema as $index => $item)
                            <tr class="hover:bg-blue-50/30 transition duration-200 group">
                                <td class="py-3.5 px-5 text-gray-400 text-sm text-center font-mono">
                                    {{ $jenisskema->firstItem() + $index }}</td>

                                <td class="py-3.5 px-5">
                                    <span
                                        class="font-mono text-sm font-medium text-gray-700">{{ $item->kode_jenisskema }}</span>
                                </td>
                                ...
                                <td class="py-3.5 px-5">
                                    <div class="flex items-center space-x-3.5">
                                        <div class="relative flex-shrink-0">
                                            <div
                                                class="w-10 h-10 rounded-xl flex items-center justify-center text-white font-semibold text-sm shadow-sm bg-gradient-to-br from-blue-500 to-indigo-500">
                                                <i class="fas fa-layer-group"></i>
                                            </div>
                                        </div>
                                        <div>
                                            <div class="font-semibold text-gray-800 text-sm">
                                                {{ $item->nama_jenisskema }}
                                            </div>
                                ...
                                <td class="py-3.5 px-5 text-gray-600 text-sm hidden lg:table-cell">
                                    <div class="flex items-center">
                                        ...
                                        <span
                                            class="truncate max-w-[150px]">{{ $item->deskripsi_jenisskema ?? '-' }}</span>
                                    </div>
                                </td>
                                ...
                                <td class="py-3.5 px-5">
                                    <div class="flex items-center justify-center space-x-1">
                                        <a href="{{ route('admin.jenisskema.show', $item->id) }}"
                                        ...
                                        <a href="{{ route('admin.jenisskema.edit', $item->id) }}"
                                        ...
                                        <form action="{{ route('admin.jenisskema.destroy', $item->id) }}" method="POST"
                                            class="inline"
                                            onsubmit="return confirmDelete(this, '{{ $item->nama_jenisskema }}')">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit"
                            ...
                            <tr>
                                <td colspan="6" class="py-16 text-center">
                                    <div class="flex flex-col items-center">
                                        ...
                                        <p class="text-gray-600 font-semibold text-lg">Belum ada data jenis skema</p>
                                        <p class="text-sm text-gray-400 mt-1">Klik tombol "Tambah Jenis Skema" untuk
                                            memulai</p>
                                        <a href="{{ route('admin.jenisskema.create') }}"
                                            class="mt-4 inline-flex items-center px-4 py-2 bg-blue-50 text-blue-600 rounded-xl hover:bg-blue-100 transition text-sm font-medium">
                                            <i class="fas fa-plus-circle mr-2"></i>
                                            Tambah Jenis Skema Sekarang
                                        </a>
                                    </div>
                                </td>
                            </tr>
            ...
            <div
                ...
                <div class="text-xs text-gray-500">
                    Menampilkan <span class="font-semibold text-gray-700">{{ $jenisskema->firstItem() ?? 0 }}</span>
                    - <span class="font-semibold text-gray-700">{{ $jenisskema->lastItem() ?? 0 }}</span>
                    dari <span class="font-semibold text-gray-700">{{ $jenisskema->total() }}</span>
                </div>
                <div>
                    {{ $jenisskema->links() }}
                </div>
