...
@section('header', 'Manajemen Jenis Reviewer')
    ...
    <div class="space-y-5">
                ...
                <div class="flex items-center space-x-4">
                    ...
                    <div>
                        ...
                        <div class="flex items-center space-x-3 mt-0.5">
                            <span class="text-purple-100 text-sm">Kelola semua data jenis reviewer</span>
                            <span
                                class="px-2.5 py-0.5 bg-white/20 backdrop-blur-sm text-white text-xs font-semibold rounded-full">
                                {{ $jenisReviewer->total() }} Jenis Reviewer
                            </span>
                        </div>
                    </div>
                </div>

                <a href="{{ route('admin.jenisreviewer.create') }}"
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            <div
                ...
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-sm text-gray-500 font-medium">Total Jenis Reviewer</p>
                ...
                <div class="mt-3 pt-3 border-t border-gray-100">
                    <span class="text-xs text-gray-400">Total seluruh jenis reviewer</span>
                </div>
            </div>
            ...
            <div
                ...
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-sm text-gray-500 font-medium">Jenis Reviewer Aktif</p>
            ...
            <div
                ...
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-sm text-gray-500 font-medium">Jenis Reviewer Nonaktif</p>
        ...
        <div
            ...
            <div class="px-5 py-3.5 border-b border-gray-100 flex flex-wrap justify-between items-center gap-3">
                ...
                <div class="flex items-center space-x-2">
                    <form method="GET" action="{{ route('admin.jenisreviewer.index') }}"
                        class="flex items-center space-x-2">
                        <div class="relative">
                            <i class="fas fa-search absolute left-3.5 top-1/2 -translate-y-1/2 text-gray-400 text-xs"></i>
                            <input type="text" name="search" value="{{ request('search') }}"
                                placeholder="Cari jenis reviewer..."
                ...
                <table class="w-full">
                    <thead>
                        <tr class="bg-gray-50/50 text-gray-500 text-xs font-semibold uppercase tracking-wider">
                            ...
                            <th class="py-3.5 px-5 text-left">Nama Jenis Reviewer</th>
                            <th class="py-3.5 px-5 text-center">Status</th>
                            <th class="py-3.5 px-5 text-center w-36">Aksi</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-gray-50">
                        @forelse($jenisReviewer as $index => $item)
                            <tr class="hover:bg-purple-50/30 transition duration-200 group">
                                <td class="py-3.5 px-5 text-gray-400 text-sm text-center font-mono">
                                    {{ $jenisReviewer->firstItem() + $index }}</td>

                                <td class="py-3.5 px-5">
                                    <span
                                        class="font-mono text-sm font-medium text-gray-700">{{ $item->kode_jenisreviewer }}</span>
                                </td>
                                ...
                                <td class="py-3.5 px-5">
                                    <div class="flex items-center justify-center space-x-1">
                                        <a href="{{ route('admin.jenisreviewer.show', $item->id) }}"
                                        ...
                                        <a href="{{ route('admin.jenisreviewer.edit', $item->id) }}"
                            ...
                            <tr>
                                <td colspan="5" class="py-16 text-center">
                                    <div class="flex flex-col items-center">
                                        ...
                                        <p class="text-gray-600 font-semibold text-lg">Belum ada data jenis reviewer</p>
                                        <p class="text-sm text-gray-400 mt-1">Klik tombol "Tambah Jenis Reviewer" untuk
                                            memulai</p>
                                        <a href="{{ route('admin.jenisreviewer.create') }}"
                                            ...
                                            Tambah Jenis Reviewer Sekarang
                                        </a>
                                    </div>
                                </td>
                            </tr>
            ...
            <div
                ...
                <div class="text-xs text-gray-500">
                    Menampilkan <span class="font-semibold text-gray-700">{{ $jenisReviewer->firstItem() ?? 0 }}</span>
                    - <span class="font-semibold text-gray-700">{{ $jenisReviewer->lastItem() ?? 0 }}</span>
                    dari <span class="font-semibold text-gray-700">{{ $jenisReviewer->total() }}</span>
                </div>
                <div>
                    {{ $jenisReviewer->links() }}
                </div>
