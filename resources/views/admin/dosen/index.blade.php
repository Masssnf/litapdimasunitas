        ...
        <div
            ...
            <div class="relative flex flex-wrap justify-between items-center gap-4">
                <div class="flex items-center space-x-4">
                    ...
                    <div>
                        <h1 class="text-2xl font-bold text-white tracking-tight">Manajemen Dosen</h1>
                        <div class="flex items-center space-x-3 mt-0.5">
                            <span class="text-blue-100 text-sm">Kelola semua data dosen</span>
                            <span
                ...
                <a href="{{ route('admin.dosen.create') }}"
                    ...
                    <span
                        class="absolute inset-0 bg-gradient-to-r from-blue-50 to-cyan-50 opacity-0 group-hover:opacity-100 transition-opacity duration-300"></span>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            <div
                ...
                <div class="mt-3 pt-3 border-t border-gray-100">
                    <span class="text-xs text-gray-400">Total seluruh dosen</span>
                </div>
            </div>
            ...
            <div
                ...
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-sm text-gray-500 font-medium">Dosen Nonaktif</p>
        ...
        <div
            ...
            <div class="px-5 py-3.5 border-b border-gray-100 flex flex-wrap justify-between items-center gap-3">
                {{-- <div class="flex items-center space-x-4">
                    <div class="flex items-center space-x-2">
                        ...
                        <span class="text-sm font-bold text-gray-800">{{ $dosen->total() }}</span>
                    </div>
                ...
                <div class="flex items-center space-x-4">
                    <h1 class="text-2xl font-bold text-blue-600 tracking-tight">Tabel Data Dosen</h1>
                </div>

                <div class="flex items-center space-x-2">
                    <form method="GET" action="{{ route('admin.dosen.index') }}" class="flex items-center space-x-2">
                        <div class="relative">
                ...
                <table class="w-full">
                    <thead>
                        <tr class="bg-gray-50/50 text-gray-500 text-xs font-semibold uppercase tracking-wider">
                            ...
                            <th class="py-3.5 px-5 text-left">NIDN</th>
                            <th class="py-3.5 px-5 text-left">Nama Dosen</th>
                            <th class="py-3.5 px-5 text-left hidden lg:table-cell">Jenis Kelamin</th>
                            <th class="py-3.5 px-5 text-left hidden xl:table-cell">Alamat</th>
                            <th class="py-3.5 px-5 text-left hidden xl:table-cell">Prodi</th>
                        ...
                        @forelse($dosen as $index => $item)
                            <tr class="hover:bg-blue-50/30 transition duration-200 group">
                                <td class="py-3.5 px-5 text-gray-400 text-sm text-center font-mono">
                                    {{ $dosen->firstItem() + $index }}</td>

                                <td class="py-3.5 px-5">
                                    <span class="font-mono text-sm font-medium text-gray-700">{{ $item->nidn }}</span>
                                </td>
                                ...
                                <td class="py-3.5 px-5">
                                    <div class="flex items-center space-x-3.5">
                                        <div class="relative flex-shrink-0">
                                            <div
                                                class="w-10 h-10 rounded-xl flex items-center justify-center text-white font-semibold text-sm shadow-sm bg-gradient-to-br from-blue-500 to-cyan-500">
                                                {{ strtoupper(substr($item->nama_dosen, 0, 1)) }}
                                            </div>
                                        </div>
                                        <div>
                                ...
                                <td class="py-3.5 px-5 text-gray-600 text-sm hidden lg:table-cell">
                                    <div class="flex items-center">
                                        <i
                                            class="fas fa-{{ $item->jenis_kelamin == 'L' ? 'mars' : 'venus' }} text-gray-300 mr-2.5 text-xs"></i>
                                ...
                                <td class="py-3.5 px-5 text-gray-600 text-sm hidden xl:table-cell">
                                    <div class="flex items-center">
                                        <i class="fas fa-map-marker-alt text-gray-300 mr-2.5 text-xs"></i>
                                        <span class="truncate max-w-[120px]">{{ $item->alamat_dosen ?? '-' }}</span>
                                    </div>
                                </td>

                                <td class="py-3.5 px-5 text-gray-600 text-sm hidden xl:table-cell">
                                    <div class="flex items-center">
                                        <i class="fas fa-book-open text-gray-300 mr-2.5 text-xs"></i>
                                        <span class="truncate max-w-[120px]">{{ $item->prodi->nama_prodi ?? '-' }}</span>
                                    </div>
                                </td>
                                ...
                                <td class="py-3.5 px-5">
                                    <div class="flex items-center justify-center space-x-1">
                                        ...
                                        <form action="{{ route('admin.dosen.destroy', $item->id) }}" method="POST"
                                            class="inline"
                                            onsubmit="return confirmDelete(this, '{{ $item->nama_dosen }}')">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit"
                            ...
                            <tr>
                                <td colspan="8" class="py-16 text-center">
                                    <div class="flex flex-col items-center">
                                        ...
                                        <p class="text-gray-600 font-semibold text-lg">Belum ada data dosen</p>
                                        <p class="text-sm text-gray-400 mt-1">Klik tombol "Tambah Dosen" untuk memulai</p>
                                        <a href="{{ route('admin.dosen.create') }}"
            ...
            <div
                ...
                <div class="text-xs text-gray-500">
                    Menampilkan <span class="font-semibold text-gray-700">{{ $dosen->firstItem() ?? 0 }}</span>
                    - <span class="font-semibold text-gray-700">{{ $dosen->lastItem() ?? 0 }}</span>
                    dari <span class="font-semibold text-gray-700">{{ $dosen->total() }}</span>
                </div>
                <div>
                    {{ $dosen->links() }}
                </div>
