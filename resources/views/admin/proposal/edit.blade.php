                ...
                <div class="flex items-center space-x-4">
                    ...
                    <div>
                        ...
                        <div class="flex items-center space-x-3 mt-0.5">
                            <span class="text-amber-100 text-sm">Ubah data proposal</span>
                            <span
            ...
            <form action="{{ route('admin.proposal.update', $proposal->id) }}" method="POST" class="p-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" name="kode_proposal"
                    ...
                    <div>
                        ...
                        <input type="number" name="dana_diusulkan"
                    ...
                    <div>
                        ...
                        <select name="status"
                            ...
                            <option value="Diajukan"
                                {{ old('status', $proposal->status) == 'Diajukan' ? 'selected' : '' }}>Diajukan</option>
                            <option value="Diverifikasi"
                                {{ old('status', $proposal->status) == 'Diverifikasi' ? 'selected' : '' }}>Diverifikasi
                            </option>
                            <option value="Direview"
                                {{ old('status', $proposal->status) == 'Direview' ? 'selected' : '' }}>Direview</option>
                            <option value="Revisi" {{ old('status', $proposal->status) == 'Revisi' ? 'selected' : '' }}>
                                Revisi</option>
                            <option value="Lolos" {{ old('status', $proposal->status) == 'Lolos' ? 'selected' : '' }}>
                                Lolos</option>
                        ...
                        <select name="periode_skema_id"
                            ...
                            @foreach ($periodeSkema as $item)
                                <option value="{{ $item->id }}"
                                    {{ old('periode_skema_id', $proposal->periode_skema_id) == $item->id ? 'selected' : '' }}>
                    ...
                    <div>
                        ...
                        <select name="ketua_dosen_id"
                            ...
                            @foreach ($dosen as $item)
                                <option value="{{ $item->id }}"
                                    {{ old('ketua_dosen_id', $proposal->ketua_dosen_id) == $item->id ? 'selected' : '' }}>
                    ...
                    <div>
                        ...
                        <select name="bidangpenelitian_id"
                            ...
                            @foreach ($bidangPenelitian as $item)
                                <option value="{{ $item->id }}"
                                    {{ old('bidangpenelitian_id', $proposal->bidangpenelitian_id) == $item->id ? 'selected' : '' }}>
                    ...
                    <div>
                        ...
                        <select name="fakultas_id"
                            ...
                            @foreach ($fakultas as $item)
                                <option value="{{ $item->id }}"
                                    {{ old('fakultas_id', $proposal->fakultas_id) == $item->id ? 'selected' : '' }}>
                    ...
                    <div>
                        ...
                        <select name="prodi_id"
                            ...
                            @foreach ($prodi as $item)
                                <option value="{{ $item->id }}"
                                    {{ old('prodi_id', $proposal->prodi_id) == $item->id ? 'selected' : '' }}>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
                    <div>
                        ...
                        <textarea name="ringkasan" rows="3"
                            class="w-full rounded-xl border-gray-300 focus:ring-amber-500 focus:border-amber-500">{{ old('ringkasan', $proposal->ringkasan) }}</textarea>
                        @error('ringkasan')
                            <p class="text-rose-500 text-sm">{{ $message }}</p>
                        @enderror
                    </div>
                    <div>
                        ...
                        <input type="text" name="kata_kunci" value="{{ old('kata_kunci', $proposal->kata_kunci) }}"
                            class="w-full rounded-xl border-gray-300 focus:ring-amber-500 focus:border-amber-500"
                ...
                <div class="mt-6 pt-4 border-t border-gray-100 flex justify-end gap-3">
                    ...
                    <button type="submit"
                        class="px-6 py-2.5 bg-gradient-to-r from-amber-500 to-orange-500 text-white rounded-xl">
