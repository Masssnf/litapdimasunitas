    ...
    <div class="space-y-5">
        ...
        <div
            ...
            <div class="relative flex flex-wrap justify-between items-center gap-4">
                <div class="flex items-center space-x-4">
                    ...
                    <div>
                        <h1 class="text-2xl font-bold text-white tracking-tight">Edit Anggota Tim</h1>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100/80 overflow-hidden">
            <div class="px-6 py-4 border-b border-gray-100">
                <h3 class="text-sm font-semibold text-gray-700">Form Edit Anggota</h3>
                <p class="text-xs text-gray-400">{{ $anggota->dosen->nama_dosen ?? '' }}</p>
            </div>
            <div class="p-6">
                <form action="{{ route('admin.proposal.anggota.update', [$proposal->id, $anggota->id]) }}" method="POST">
                    @csrf
                    @method('PUT')
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="text" value="{{ $anggota->dosen->nama_dosen ?? '-' }}"
                        ...
                        <div>
                            ...
                            <select name="peran"
                                ...
                                required>
                                <option value="ketua" {{ old('peran', $anggota->peran) == 'ketua' ? 'selected' : '' }}>
                                    Ketua</option>
                                <option value="anggota" {{ old('peran', $anggota->peran) == 'anggota' ? 'selected' : '' }}>
                                    Anggota</option>
                            </select>
                            @error('peran')
