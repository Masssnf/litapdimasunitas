...
<div class="space-y-5">
        ...
        <div class="relative flex flex-wrap justify-between items-center gap-4">
            ...
            <a href="{{ route('admin.proposal.create') }}" 
               class="group relative px-5 py-2.5 bg-white text-indigo-600 font-semibold rounded-xl shadow-lg hover:shadow-2xl transition-all duration-300 flex items-center text-sm overflow-hidden">
                <span class="absolute inset-0 bg-gradient-to-r from-indigo-50 to-purple-50 opacity-0 group-hover:opacity-100 transition-opacity duration-300"></span>
    ...
    <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100/80 p-4 hover:shadow-md transition-all duration-300">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-xs text-gray-500 font-medium">Total</p>
                    <p class="text-xl font-bold text-gray-800 mt-0.5">{{ $total }}</p>
                </div>
                <div class="w-10 h-10 bg-gradient-to-br from-gray-500 to-gray-600 rounded-xl flex items-center justify-center shadow-lg shadow-gray-500/25">
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100/80 p-4 hover:shadow-md transition-all duration-300">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-xl font-bold text-gray-600 mt-0.5">{{ $draft }}</p>
                </div>
                <div class="w-10 h-10 bg-gradient-to-br from-gray-400 to-gray-500 rounded-xl flex items-center justify-center shadow-lg shadow-gray-500/25">
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100/80 p-4 hover:shadow-md transition-all duration-300">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-xl font-bold text-blue-600 mt-0.5">{{ $diajukan }}</p>
                </div>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100/80 p-4 hover:shadow-md transition-all duration-300">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-xl font-bold text-yellow-600 mt-0.5">{{ $direview }}</p>
                </div>
                <div class="w-10 h-10 bg-gradient-to-br from-yellow-500 to-yellow-600 rounded-xl flex items-center justify-center shadow-lg shadow-yellow-500/25">
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100/80 p-4 hover:shadow-md transition-all duration-300">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-xl font-bold text-orange-600 mt-0.5">{{ $revisi }}</p>
                </div>
                <div class="w-10 h-10 bg-gradient-to-br from-orange-500 to-orange-600 rounded-xl flex items-center justify-center shadow-lg shadow-orange-500/25">
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100/80 p-4 hover:shadow-md transition-all duration-300">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-xl font-bold text-emerald-600 mt-0.5">{{ $diterima }}</p>
                </div>
            ...
            <div class="flex items-center space-x-2">
                <form method="GET" action="{{ route('admin.proposal.index') }}" class="flex items-center space-x-2">
                    ...
                    <select name="status" 
                        ...
                        <option value="direview" {{ request('status') == 'direview' ? 'selected' : '' }}>Di Review</option>
                        <option value="diterima" {{ request('status') == 'diterima' ? 'selected' : '' }}>Diterima</option>
                        <option value="ditolak" {{ request('status') == 'ditolak' ? 'selected' : '' }}>Ditolak</option>
                        <option value="revisi" {{ request('status') == 'revisi' ? 'selected' : '' }}>Revisi</option>
                    </select>

                    <button type="submit" class="w-9 h-9 rounded-xl bg-indigo-500 hover:bg-indigo-600 text-white transition flex items-center justify-center">
                        <i class="fas fa-search text-sm"></i>
                    </button>
                    @if(request('search') || request('status'))
                        <a href="{{ route('admin.proposal.index') }}" class="w-9 h-9 rounded-xl bg-gray-100 hover:bg-gray-200 text-gray-500 hover:text-gray-700 transition flex items-center justify-center">
                ...
                <tbody class="divide-y divide-gray-50">
                    @forelse($proposal as $index => $item)
                        <tr class="hover:bg-indigo-50/30 transition duration-200 group">
                            <td class="py-3.5 px-5 text-gray-400 text-sm text-center font-mono">{{ $proposal->firstItem() + $index }}</td>

                            <td class="py-3.5 px-5">
                                <span class="font-mono text-sm font-medium text-gray-700">{{ $item->kode_proposal }}</span>
                            </td>

                            <td class="py-3.5 px-5">
                                <div class="flex items-center space-x-3.5">
                                    <div class="relative flex-shrink-0">
                                        <div class="w-10 h-10 rounded-xl flex items-center justify-center text-white font-semibold text-sm shadow-sm bg-gradient-to-br from-indigo-500 to-purple-500">
                            ...
                            <td class="py-3.5 px-5 text-gray-600 text-sm hidden lg:table-cell">
                                <div class="flex items-center">
                                    ...
                                    <span class="truncate max-w-[120px]">{{ $item->ketuaDosen->nama_dosen ?? '-' }}</span>
                                </div>
                            </td>

                            <td class="py-3.5 px-5 text-gray-600 text-sm text-center hidden xl:table-cell">
                                <span class="font-semibold text-emerald-600">{{ $item->dana_diusulkan_formatted }}</span>
                            </td>
                            ...
                            <td class="py-3.5 px-5">
                                <div class="flex items-center justify-center space-x-1">
                                    ...
                                    <form action="{{ route('admin.proposal.destroy', $item->id) }}" method="POST" class="inline" onsubmit="return confirmDelete(this, '{{ $item->judul }}')">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit"
                        ...
                        <tr>
                            <td colspan="7" class="py-16 text-center">
                                <div class="flex flex-col items-center">
                                    ...
                                    <p class="text-sm text-gray-400 mt-1">Klik tombol "Tambah Proposal" untuk memulai</p>
                                    <a href="{{ route('admin.proposal.create') }}" class="mt-4 inline-flex items-center px-4 py-2 bg-indigo-50 text-indigo-600 rounded-xl hover:bg-indigo-100 transition text-sm font-medium">
