...
@section('header', 'Manajemen Skema')
        ...
        <div
            ...
            <div class="relative flex flex-wrap justify-between items-center gap-4">
                <div class="flex items-center space-x-4">
                    ...
                    <div>
                        <h1 class="text-2xl font-bold text-white tracking-tight">Manajemen Skema</h1>
                        <div class="flex items-center space-x-3 mt-0.5">
                            <span class="text-purple-100 text-sm">Kelola semua data skema</span>
                            <span
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            <div
                ...
                <div class="mt-3 pt-3 border-t border-gray-100">
                    <span class="text-xs text-gray-400">Total seluruh skema</span>
                </div>
            </div>
            ...
            <div
                ...
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-sm text-gray-500 font-medium">Skema Aktif</p>
            ...
            <div
                ...
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-sm text-gray-500 font-medium">Skema Nonaktif</p>
            ...
            <div
                ...
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-sm text-gray-500 font-medium">Jenis Skema</p>
                        <p class="text-2xl font-bold text-blue-600 mt-1">{{ $jenisSkema->count() }}</p>
                    </div>
                    <div
                ...
                <div class="mt-3 pt-3 border-t border-gray-100">
                    <span class="text-xs text-blue-500">● Total jenis skema</span>
                </div>
            </div>
        </div>
        ...
        <div
            ...
            <div class="px-5 py-3.5 border-b border-gray-100 flex flex-wrap justify-between items-center gap-3">
                <div class="flex items-center space-x-4">
                    <div class="flex items-center space-x-2">
                        ...
                        <span class="text-sm font-bold text-gray-800">{{ $skema->total() }}</span>
                    </div>
                ...
                <div class="flex items-center space-x-2">
                    <form method="GET" action="{{ route('admin.skema.index') }}" class="flex items-center space-x-2">
                        <div class="relative">
                ...
                <table class="w-full">
                    <thead>
                        <tr class="bg-gray-50/50 text-gray-500 text-xs font-semibold uppercase tracking-wider">
                            ...
                            <th class="py-3.5 px-5 text-left">Nama Skema</th>
                            <th class="py-3.5 px-5 text-left hidden lg:table-cell">Jenis Skema</th>
                            <th class="py-3.5 px-5 text-left hidden xl:table-cell">Dana</th>
                            <th class="py-3.5 px-5 text-left hidden xl:table-cell">Durasi</th>
                        ...
                        @forelse($skema as $index => $item)
                            <tr class="hover:bg-purple-50/30 transition duration-200 group">
                                <td class="py-3.5 px-5 text-gray-400 text-sm text-center font-mono">
                                    {{ $skema->firstItem() + $index }}</td>

                                <td class="py-3.5 px-5">
                                    <span
                                        class="font-mono text-sm font-medium text-gray-700">{{ $item->kode_skema }}</span>
                                </td>
                                ...
                                <td class="py-3.5 px-5 text-gray-600 text-sm hidden lg:table-cell">
                                    <div class="flex items-center">
                                        ...
                                        <span
                                            class="truncate max-w-[120px]">{{ $item->jenisskema->nama_jenisskema ?? '-' }}</span>
                                    </div>
                                </td>

                                <td class="py-3.5 px-5 text-gray-600 text-sm hidden xl:table-cell">
                                    <div class="flex items-center">
                                        <i class="fas fa-money-bill-wave text-gray-300 mr-2.5 text-xs"></i>
                                        <span>Rp {{ number_format($item->dana_minimalskema ?? 0, 0, ',', '.') }} - Rp
                                            {{ number_format($item->dana_maksimalskema ?? 0, 0, ',', '.') }}</span>
                                    </div>
                                </td>

                                <td class="py-3.5 px-5 text-gray-600 text-sm hidden xl:table-cell">
                                    <div class="flex items-center">
                                        <i class="fas fa-clock text-gray-300 mr-2.5 text-xs"></i>
                                        <span>{{ $item->durasi_bulan ?? '-' }} Bulan</span>
                                    </div>
                                </td>
                                ...
                                <td class="py-3.5 px-5">
                                    <div class="flex items-center justify-center space-x-1">
                                        <a href="{{ route('admin.skema.show', $item->id) }}"
                                        ...
                                        <form action="{{ route('admin.skema.destroy', $item->id) }}" method="POST"
                                            class="inline"
                                            onsubmit="return confirmDelete(this, '{{ $item->nama_skema }}')">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit"
                            ...
                            <tr>
                                <td colspan="8" class="py-16 text-center">
                                    <div class="flex flex-col items-center">
                                        ...
                                        <p class="text-gray-600 font-semibold text-lg">Belum ada data skema</p>
                                        <p class="text-sm text-gray-400 mt-1">Klik tombol "Tambah Skema" untuk memulai</p>
                                        <a href="{{ route('admin.skema.create') }}"
                                            class="mt-4 inline-flex items-center px-4 py-2 bg-purple-50 text-purple-600 rounded-xl hover:bg-purple-100 transition text-sm font-medium">
                                            <i class="fas fa-plus-circle mr-2"></i>
                                            Tambah Skema Sekarang
                                        </a>
                                    </div>
                                </td>
                            </tr>
            ...
            <div
                ...
                <div class="text-xs text-gray-500">
                    Menampilkan <span class="font-semibold text-gray-700">{{ $skema->firstItem() ?? 0 }}</span>
                    - <span class="font-semibold text-gray-700">{{ $skema->lastItem() ?? 0 }}</span>
                    dari <span class="font-semibold text-gray-700">{{ $skema->total() }}</span>
                </div>
                <div>
                    {{ $skema->links() }}
                </div>
